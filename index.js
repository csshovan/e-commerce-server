  ...
  try {
    ...
    app.post("/order", async (req, res) => {
      ...
      const data = {
        ...
        success_url: `http://localhost:5000/payment/success/${tran_id}?cartItems=${order?.cartItems}&productItems=${order?.productItems}&itemQuantity=${order?.itemQuantity}`,
    ...
    app.post("/refundpayment", async (req, res) => {
      ...
      const data = {
        ...
        currency: "BDT",
        tran_id: tran_id, // use unique tran_id for each api call
        success_url: `http://localhost:5000/payments/success/${tran_id}?refundId=${refundInfo.refundId}`,
        fail_url: "http://localhost:3030/fail",
        cancel_url: "http://localhost:3030/cancel",
        ipn_url: "http://localhost:3030/ipn",
        shipping_method: "Courier",
      ...
      app.post("/payments/success/:tranId", async (req, res) => {
        ...
        if (result.modifiedCount > 0) {
          ...
          res.redirect(`http://localhost:5173/dashboard/manage-refunds`);
        }
      });
    });
